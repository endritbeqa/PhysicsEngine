...
#include "Physics/include/forces/SpringForce.h"
...
SpringForce::SpringForce(double ks, double kd, double l){
    type = N_ary;
    springConstant = ks;
...
void SpringForce::apply() {
    ...
    double coeff = -(springConstant * (l_norm - length) + (dampeningConstant * l_dt.dot(l)) / l_norm);
